...
or distribute this program undesr any other version of the
...
public class MobPacket {
    ...
    public static byte[] moveMonster(boolean useskill, int mode, int skillId, int skillLevel, int effectDelay, int oid, Point startPos, List<LifeMovementFragment> moves) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        mplew.writeInt(0); // size + int int
        mplew.writeInt(0); // size + int
    ...
    public static byte[] spawnMonster(MapleMonster life, int spawnType, int link) {
        ...
        mplew.writeShort(life.getOriginFh()); // Origin FH
    ...
    public static byte[] controlMonster(MapleMonster life, boolean newSpawn, boolean aggro) {
        ...
        mplew.write(life.getCarnivalTeam());
        mplew.writeInt(0); //big bang - another int here
    ...
    public static byte[] moveMonsterResponse(int objectid, short moveid, int currentMp, boolean aggro/*useSkills*/, int skillId, int skillLevel) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.MOVE_MONSTER_RESPONSE.getValue());
        mplew.writeInt(objectid);
        mplew.writeShort(moveid);
        mplew.write(aggro/*useSkills ? 1 : 0*/);
...
//	mplew.writeInt(0);
    ...
    public static byte[] applyMonsterStatus(final int oid, final MonsterStatus mse, int x, MobSkill skil, long duration, int effectDelay) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.APPLY_MONSTER_STATUS.getValue());
        mplew.writeInt(oid);
        PacketHelper.writeSingleMaskMob(mplew, mse);
        mplew.writeShort(x);
        mplew.writeShort(skil.getSkillId());
        mplew.writeShort(skil.getSkillLevel());
        mplew.writeShort(mse.isEmpty() ? 1 : 0/*(int) duration / 1000*//*mse.isEmpty() ? 1 : 0*/); // 10 = 1秒 // might actually be the buffTime but it's not displayed anywhere
        mplew.writeShort(effectDelay); // delay in ms 1000 = 1秒
        mplew.write(1); // size
        mplew.write(1); // ? v97
    ...
    public static byte[] applyMonsterStatus(final MapleMonster mons, final MonsterStatusEffect ms, long duration) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.APPLY_MONSTER_STATUS.getValue());
        mplew.writeInt(mons.getObjectId());
        //aftershock extra int here
        PacketHelper.writeSingleMaskMob(mplew, ms.getStati());
        ...
        mplew.writeShort(ms.getStati().isEmpty() ? 1 : 0/*ms.getStati().isEmpty() ? 1 : 0*/); // 持續時間 10 = 1秒 // might actually be the buffTime but it's not displayed anywhere
        mplew.writeShort(0); // delay in ms 1000 = 1秒
        mplew.write(1); // size
        mplew.write(1); // ? v97
...
/*    public static byte[] applyMonsterStatus(final MapleMonster mons, final List<MonsterStatusEffect> mse) {
	if (mse.size() <= 0 || mse.get(0) == null) {
	    ...
	    PacketHelper.writeMobSingleMask(mplew, MonsterStatus.EMPTY);
	    ...
	    PacketHelper.writeMobSingleMask(mplew, ms.getStati());
...
/*    public static byte[] applyMonsterStatus(final int oid, final Map<MonsterStatus, Integer> stati, final List<Integer> reflection, MobSkill skil) {
        ...
        mplew.writeShort(SendPacketOpcode.APPLY_MONSTER_STATUS.getValue());
        mplew.writeInt(oid);
        PacketHelper.writeMobMask(mplew, stati.keySet());
...
//        mplew.write(1); // ? v97
    ...
    public static byte[] cancelMonsterStatus(int oid, MonsterStatus stat) {
        ...
        mplew.writeInt(oid);
        PacketHelper.writeSingleMaskMob(mplew, stat);
        mplew.write(1); // size // reflector is 3~!??
        mplew.write(2); // ? v97
