...
public class EventManager {
    ...
    public void warpAllPlayer(int from, int to, int spawnpoint) {
        final MapleMap tomap = getMapFactory().getMap(to);
        final MapleMap frommap = getMapFactory().getMap(from);
        List<MapleCharacter> list = frommap.getCharactersThreadsafe();
        if (tomap != null && frommap != null && list != null && frommap.getCharactersSize() > 0) {
            for (MapleMapObject mmo : list) {
                ((MapleCharacter) mmo).changeMap(tomap, tomap.getPortal(spawnpoint));
            }
        }
    }
    ...
    public void broadcastShip(final int mapid, final int effect, final int effect2) {
        getMapFactory().getMap(mapid).broadcastMessage(MaplePacketCreator.boatPacket(effect, effect2));
    }
