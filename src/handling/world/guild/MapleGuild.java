...
//		broadcast(MaplePacketCreator.guildContribution(id, cid, mg.getGuildContribution()));
            }
        }
        gp += amount;
        level = calculateLevel();
    ...
    public boolean activateSkill(int skill, String name) {
        ...
        broadcast(MaplePacketCreator.guildSkillPurchased(id, skill, ourSkill.level, ourSkill.timestamp, ourSkill.purchaser, name));
        return true;
    }
...
//        gainGP(1000, true, cid);
        broadcast(MaplePacketCreator.guildSkillPurchased(id, skill, ourSkill.level, ourSkill.timestamp, name, name));
        return true;
    }
    ...
    public static final MapleGuildResponse sendInvite(final MapleClient c, final String targetName) {
        ...
        mc.getClient().announce(MaplePacketCreator.guildInvite(c.getPlayer().getGuildId(), c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob()));
        return null;
    }
