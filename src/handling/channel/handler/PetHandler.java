...
public class PetHandler {
        ...
        if (Randomizer.nextInt(99) <= petCommand.getProbability()) {
            ...
            if (pet.getCloseness() < 30000) {
                ...
                if (newCloseness >= GameConstants.getClosenessNeededForLevel(pet.getLevel() + 1)) {
                    ...
                    c.announce(PetPacket.showOwnPetLevelUp());
                    chr.getMap().broadcastMessage(PetPacket.showPetLevelUp(chr));
                }
                c.announce(PetPacket.updatePet(pet, chr.getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
            }
        }
        chr.getMap().broadcastMessage(PetPacket.commandResponse(chr.getId(), (byte) petCommand.getSkillId(), success, false));
    }
    ...
    public static final void MovePet(final LittleEndianAccessor slea, final MapleCharacter chr) {
        final int petId = 0/*GameConstants.GMS ? ((int)slea.readLong()) : slea.readInt()*/;
        ...
        if (res != null && chr != null && res.size() != 0 && chr.getMap() != null) { // map crash hack
            ...
            chr.getMap().broadcastMessage(chr, PetPacket.movePet(chr.getId(), pos, (byte)petId, res), false);
  	    if (chr.hasBlockedInventory() || chr.getStat().pickupRange <= 0.0 || chr.inPVP()) {
	        return;
	    }
	    chr.setScrolledPosition((short) 0);
	    List<MapleMapObject> objects = chr.getMap().getMapObjects(Arrays.asList(MapleMapObjectType.ITEM));
