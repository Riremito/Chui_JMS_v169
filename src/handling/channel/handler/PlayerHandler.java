...
/*        } else if (vip >= 2) {
        ...
        c.announce(MTSCSPacket.getTrockRefresh(chr, vip, addrem == 3));
    }
...
//        c.announce(MaplePacketCreator.enableActions());
        if (player != null/* && !player.isClone()*/) {
            if (!player.isGM() || c.getPlayer().isGM()) {
                c.announce(MaplePacketCreator.charInfo(player, c.getPlayer().getId() == objectid));
            }
        }
    }
...
//        slea.skip(2);
        ...
        if (type != -2 && type != -3 && type != -4) { // Not map damage
            ...
            slea.readByte();
            slea.readByte();
            if (chr.getBuffedValue(MapleBuffStat.POWERGUARD) != null && reflect > 0) {
                reflect = slea.readByte();
                /*oid = */slea.readInt();
                unk = slea.readByte(); // 成功時是0x06
                pos = slea.readPos();
            }
            slea.readByte();
            attacker = chr.getMap().getMonsterByOid(oid); // oid
        ...
        } else if (damage < -1 || damage > 20000) {
            System.out.println(chr.getName() + " damage < -1 || damage > 20000");
        ...
        if (chr.getStat().dodgeChance > 0 && Randomizer.nextInt(100) < chr.getStat().dodgeChance) {
            c.announce(MaplePacketCreator.showSpecialEffect(17)); //lol
            return;
        }
...
/*            if (chr.getBuffedValue(MapleBuffStat.POWERGUARD) != null  && reflect > 0 && attacker != null && damage != 0) {
                long bouncedamage = (long) (damage * (chr.getBuffedValue(MapleBuffStat.POWERGUARD).doubleValue() / 100));
                bouncedamage = Math.min(bouncedamage, attacker.getMobMaxHp() / 10);
                attacker.damage(chr, bouncedamage, true);
                chr.getMap().broadcastMessage(chr, MobPacket.damageMonster(oid, bouncedamage), chr.getTruePosition());
                damage -= bouncedamage;
            }*/
...
//        c.announce(MaplePacketCreator.enableActions());
        chr.getMap().broadcastMessage(chr, MaplePacketCreator.damagePlayer(chr.getId(), type, damage, monsteridfrom, direction, reflect, oid, unk, pos), false);
    }
    ...
    public static final void AranCombo(final MapleClient c, final MapleCharacter chr, int toAdd) {
        if (chr != null && chr.getJob() >= 2000 && chr.getJob() <= 2112) {
            ...
            c.announce(MaplePacketCreator.testCombo(combo));
...
/*            case 4121006:
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (attack.skill == 1211002) { // handle charged blow
                ...
                if (chr.getJob() == 412 && bulletConsume > 0 && ipp.getQuantity() < MapleItemInformationProvider.getInstance().getSlotMax(projectile)) {
                    ...
                    if (chr.getTotalSkillLevel(expert) > 0) {
                        ...
                        if (eff.makeChanceResult()) {
                            ...
                            c.announce(MaplePacketCreator.updateInventorySlot(MapleInventoryType.USE, ipp, false));
                            bulletConsume = 0; //regain a star after using
                            c.announce(MaplePacketCreator.getInventoryStatus());
                        }
                    }
                }
                if (bulletConsume > 0) {
...
//        chr.getCheatTracker().checkDrop(true);
    ...
    public static final void ChangeEmotion(final int emote, final MapleCharacter chr) {
        if (emote > 7) {
            final int emoteid = 5159992 + emote; // 5160000開始
//            final int emoteid = 4039992 + emote; // 4040000開始
...
//        System.out.println("client 請求補 HP " + healHP + " MP " + healMP);
        ...
        if (healHP != 0 && chr.canHP(now + 1000)) {
            ...
            chr.addHP(healHP);
//            System.out.println("補了 HP " + healHP);
        }
        if (healMP != 0 && chr.canMP(now + 1000)) { //just for lag
            ...
            chr.addMP(healMP);
//            System.out.println("補了 MP " + healMP);
        }
    }
    ...
    public static final void MovePlayer(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (res != null && res.size() > 0) { // TODO more validation of input data
...
/*                } else {
                    c.announce(MTSCSPacket.useWheel((byte) (chr.getInventory(MapleInventoryType.CASH).countById(5510000) - 1)));
