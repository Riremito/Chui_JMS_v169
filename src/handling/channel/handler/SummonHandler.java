...
//        final MapleSummon sum = chr.getSummons().get(slea.readInt());
        ...
        if (sum == null || sum.getOwnerId() != chr.getId() || sum.getSkillLevel() <= 0 || sum.getMovementType() == SummonMovementType.STATIONARY) {
            return;
        }
        ...
        if (res != null && res.size() > 0) {
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.moveSummon(chr.getId(), sum.getSkill()/*sum.getObjectId()*/, pos, res), false);
        }
    }

    public static final void DamageSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
//        System.out.println(slea.toString());
        final byte unkByte = slea.readByte();
        final int damage = slea.readInt();
        final int monsterIdFrom = slea.readInt();
        final int stance = slea.readByte(); // stance
        ...
        try {
            while (iter.hasNext()) {
                ...
                if (summon.isPuppet() && summon.getOwnerId() == chr.getId() && damage > 0) { //We can only have one puppet(AFAIK O.O) so this check is safe.
                    ...
                    chr.getMap().broadcastMessage(chr, MaplePacketCreator.damageSummon(chr.getId(), summon.getSkill(), damage, unkByte, monsterIdFrom, stance), summon.getTruePosition());
                    break;
                }
            }
        } finally {
...
/*        if (sse != null && numAttacked > sse.mobCount) {
            chr.dropMessage(0, "Warning: Attacking more monster than summon can do");
        ...
        slea.skip(8/*summon.getSkill() == 35111002 ? 24 : 12*/); //some pos stuff
...
//                continue;
        ...
        map.broadcastMessage(chr, MaplePacketCreator.summonAttack(summon.getOwnerId(), summon.getSkill()/*summon.getObjectId()*/, animation, allDamage, chr.getLevel(), false), summon.getTruePosition());
        //}
        ...
        for (Pair<Integer, Integer> attackEntry : allDamage) {
            ...
            if (chr.isGM() || toDamage < (chr.getStat().getCurrentMaxBaseDamage() * 5.0 * (summonEffect.getSelfDestruction() + summonEffect.getDamage() + chr.getStat().getDamageIncrease(summonEffect.getSourceId())) / 100.0)) { //10 x dmg.. eh
                ...
                if (!mob.isAlive()) {
                    chr.getClient().announce(MobPacket.killMonster(mob.getObjectId(), 1));
                }
            } else {
...
//            chr.removeVisibleMapObject(summon);
    ...
    public static final void RemoveSummon(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        final MapleSummon summon = (MapleSummon) obj;
//        final MapleSummon summon = c.getPlayer().getSummons().get(slea.readInt());
...
//        final MapleSummon sum = chr.getSummons().get(slea.readInt());
        ...
        switch (sum.getSkill()) {
            ...
            case 35111011: //healing
                ...
                chr.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, chr.getLevel(), sum.getSkillLevel()));
            ...
            case 1321007: //beholder
                ...
                chr.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, chr.getLevel(), bHealingLvl));
...
/*        if (GameConstants.isAngel(sum.getSkill())) {
            ...
            chr.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, 2, 1));
    ...
    public static final void SummonPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (MapleMapObject mo : chr.getMap().getCharactersIntersect(box)) {
            ...
            if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                ...
                chr.getClient().announce(MaplePacketCreator.getPVPHPBar(attacked.getId(), attacked.getStat().getHp(), attacked.getStat().getCurrentMaxHp()));
        ...
        if (killed || addedScore > 0) {
            ...
            chr.getClient().announce(MaplePacketCreator.getPVPScore(ourScore + addedScore, killed));
        }
        if (didAttack) {
            chr.getMap().broadcastMessage(MaplePacketCreator.pvpSummonAttack(chr.getId(), chr.getLevel(), summon.getSkill()/*summon.getObjectId()*/, summon.isFacingLeft() ? 4 : 0x84, summon.getTruePosition(), ourAttacks));
            if (!summon.isMultiAttack()) {
                chr.getMap().broadcastMessage(MaplePacketCreator.removeSummon(summon, true));
                chr.getMap().removeMapObject(summon);
//                chr.removeVisibleMapObject(summon);
