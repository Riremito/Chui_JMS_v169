...
public class ChatHandler {
    ...
    public static final void GeneralChat(final String text/*, final byte unk*/, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null) {
            if (/*c.getPlayer().isSuperGM() && */CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
                return;
            }
            if (!chr.isIntern() && text.length() >= 80) {
                return;
            }
            if ("e".equals(text) || "E".equals(text)) {
                c.removeClickedNPC();
                NPCScriptManager.getInstance().dispose(c);
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
//            chr.getMap().broadcastMessage(MaplePacketCreator.serverNotice(2, chr.getName() + " : 1~" + "投擲骰子，擲出" + "。"), c.getPlayer().getTruePosition());
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                if (chr.isHidden()) {
                    chr.getMap().broadcastGMMessage(chr, MaplePacketCreator.getChatText(chr.getId(), text, false/*c.getPlayer().isSuperGM()*//*, unk*/), true);
                } else {
//                    chr.getCheatTracker().checkMsg();
                    chr.getMap().broadcastMessage(MaplePacketCreator.getChatText(chr.getId(), text, false/*c.getPlayer().isSuperGM()*//*, unk*/), c.getPlayer().getTruePosition());
                }
        ...
        switch (slea.readByte()) {
                ...
                if (messenger != null) {
                    ...
                    if (target != null) {
                        if (target.getMessenger() == null) {
                            ...
                            c.announce(MaplePacketCreator.messengerChat(c.getPlayer().getName() + " : " + target.getName() + " is already using Maple Messenger."));
                        }
                    } else {
            ...
            case 0x05: // decline
                ...
                if (target != null) { // This channel
                    if (target.getMessenger() != null) {
                        target.getClient().announce(MaplePacketCreator.messengerNote(c.getPlayer().getName(), 5, 0));
                    }
                } else { // Other channel
            ...
            case 5: { // Find
                ...
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
                        c.announce(MaplePacketCreator.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 68));
                    } else {
			    ...
			    break;
                        ...
                        if (player != null) {
                            if (!player.isIntern() || (c.getPlayer().isIntern() && player.isIntern())) {
                                c.announce(MaplePacketCreator.getFindReply(recipient, (byte) ch, mode == 68));
                            } else {
                                c.announce(MaplePacketCreator.getWhisperReply(recipient, (byte) 0));
                            }
                            return;
                    ...
                    if (ch == -10) {
                        c.announce(MaplePacketCreator.getFindReplyWithCS(recipient, mode == 68));
                    } else if (ch == -20) {
