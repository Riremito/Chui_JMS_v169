    ...
    public static final void DenyPartyRequest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (c.getPlayer().getParty() == null && c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_INVITE)) == null) {
            ...
            if (party != null) {
                ...
                if (action == (GameConstants.GMS ? 0x1D : 0x1B)) { //accept
                ...
                } else if (action != (GameConstants.GMS ? 0x18 : 0x16)) {
        ...
        switch (operation) {
            case 1: // create
                if (party == null) {
                    ...
                    c.getPlayer().setParty(party);
                    c.announce(MaplePacketCreator.partyCreated(party.getId(), partyplayer));
                    // 開時空門之後開組隊傳這個才不會回去的位置錯誤
//                    c.getPlayer().silentPartyUpdate();
//                    c.getPlayer().getClient().announce(MaplePacketCreator.updateParty(c.getChannel(), c.getPlayer().getParty(), PartyOperation.SILENT_UPDATE, null));
//                    c.getPlayer().updatePartyMemberHP();
//                    c.getPlayer().receivePartyMemberHP();
                } else {
                    if (party.getExpeditionId() > 0) {
...
//                            c.announce(MaplePacketCreator.partyStatusMessage(22, invited.getName()));
                            invited.getClient().announce(MaplePacketCreator.partyInvite(c.getPlayer()));
                        } else {
            ...
            case 6: // change leader
                c.getPlayer().dropMessage(5, "変更リーダー機能は利用不可");
/*                if (party != null) {
                    if (party.getExpeditionId() > 0) {
                        c.getPlayer().dropMessage(5, "You may not do party operations while in a raid.");
                        return;
                ...
                if (GameConstants.GMS) {
                    ...
                    if (party != null && party.getMembers().size() < 6) {
                        ...
                        if (cfrom != null && cfrom.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_REQUEST)) == null) {
                            c.announce(MaplePacketCreator.partyStatusMessage(50, c.getPlayer().getName()));
                            cfrom.getClient().announce(MaplePacketCreator.partyRequestInvite(c.getPlayer()));
                        } else {
        ...
        if (c.getPlayer().getSidekick() == null && action == 0x5A) { //accept
            ...
            if (party != null) {
                ...
                if (sid <= 0) {
                    ...
                    c.announce(MaplePacketCreator.updateSidekick(c.getPlayer(), s, true));
                    party.setSidekick(s);
                    party.getClient().announce(MaplePacketCreator.updateSidekick(party, s, true));
                }
            } else {
            ...
            case 0x41: // create
                if (c.getPlayer().getSidekick() == null) {
                    ...
                    if (other.getSidekick() == null && MapleSidekick.checkLevels(c.getPlayer().getLevel(), other.getLevel())) {
                        other.getClient().announce(MaplePacketCreator.sidekickInvite(c.getPlayer()));
    ...
    public static final void PartySearchStart(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer().isInBlockedMap() || FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())) {
            ...
            c.announce(MaplePacketCreator.showMemberSearch(c.getPlayer().getMap().getCharactersThreadsafe()));
            return;
        } else if (c.getPlayer().getParty() == null) {
            MapleParty party = World.Party.createParty(new MaplePartyCharacter(c.getPlayer()));
            c.getPlayer().setParty(party);
            c.announce(MaplePacketCreator.partyCreated(party.getId(), party.getLeader()));
        } else {
    ...
    public static final void PartySearchStop(final LittleEndianAccessor slea, final MapleClient c) {
        if (GameConstants.GMS) {
            ...
            c.announce(MaplePacketCreator.showPartySearch(parties));
        }
    }
        ...
        switch (mode) {
            ...
            case -105:
                ...
                if (c.getPlayer().getParty() == null && World.Party.searchParty(pst).size() < 10) {
                    ...
                    if (pst.exped) {
                        c.announce(MaplePacketCreator.expeditionStatus(World.Party.getExped(party.getExpeditionId()), true));
                    }
                    c.announce(MaplePacketCreator.partyListingAdded(ps));
                } else {
            ...
            case -103:
                ...
                c.announce(MaplePacketCreator.getPartyListing(pst));
                break;
            case 84: //close
            case 0xA2:
            case -94:
                        ...
                        if (exped != null) {
                            ...
                            if (ps != null && c.getPlayer().getLevel() <= ps.getType().maxLevel && c.getPlayer().getLevel() >= ps.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                                ...
                                if (partyId < 0) {
                                    ...
                                    c.getPlayer().setParty(party);
                                    c.announce(MaplePacketCreator.partyCreated(party.getId(), partyplayer));
                                    c.announce(MaplePacketCreator.expeditionStatus(exped, true));
                                ...
                                c.announce(MaplePacketCreator.expeditionError(0, c.getPlayer().getName()));
                            }
                        }
                    }
                }
    ...
    public static final void Expedition(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) {
            case 42:
            case 0x31: //create [PartySearchID]
            case 119:
                ...
                if (et != null && c.getPlayer().getParty() == null && c.getPlayer().getLevel() <= et.maxLevel && c.getPlayer().getLevel() >= et.minLevel) {
                    ...
                    c.announce(MaplePacketCreator.expeditionError(0, ""));
                }
                break;
                ...
                if (theCh > 0) {
                    ...
                    if (invited != null && invited.getParty() == null && party != null && party.getExpeditionId() > 0) {
                        ...
                        if (me != null && me.getAllMembers() < me.getType().maxMembers && invited.getLevel() <= me.getType().maxLevel && invited.getLevel() >= me.getType().minLevel) {
                            c.announce(MaplePacketCreator.expeditionError(7, invited.getName()));
                            invited.getClient().announce(MaplePacketCreator.expeditionInvite(c.getPlayer(), me.getType().exped));
                        } else {
                            c.announce(MaplePacketCreator.expeditionError(3, invited.getName()));
                        }
                    } else {
                        c.announce(MaplePacketCreator.expeditionError(2, name));
                    }
                } else {
                    c.announce(MaplePacketCreator.expeditionError(0, name));
                }
                break;
                    ...
                    if (cfrom != null && cfrom.getParty() != null && cfrom.getParty().getExpeditionId() > 0) {
                        ...
                        if (exped != null && action == 8) {
                            if (c.getPlayer().getLevel() <= exped.getType().maxLevel && c.getPlayer().getLevel() >= exped.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                                ...
                                c.announce(MaplePacketCreator.expeditionError(3, cfrom.getName()));
                            }
                        } else if (action == 9) {
                            cfrom.getClient().announce(MaplePacketCreator.partyStatusMessage(23, c.getPlayer().getName()));
                        }
                                    ...
                                    if (expelled != null && expelled.isOnline()) {
                                        ...
                                        if (partyIndexTo < exped.getParties().size()) { //already exists
                                            ...
                                            if (party != null && party.getMembers().size() < 6) {
                                                ...
                                                chr.receivePartyMemberHP();
                                                chr.updatePartyMemberHP();
                                                chr.getClient().announce(MaplePacketCreator.expeditionStatus(exped, true));
                                            }
                                        } else {
                                            party = World.Party.createPartyAndAdd(expelled, exped.getId());
                                            chr.setParty(party);
                                            chr.getClient().announce(MaplePacketCreator.partyCreated(party.getId(), party.getLeader()));
                                            chr.getClient().announce(MaplePacketCreator.expeditionStatus(exped, true));
                                            World.Party.expedPacket(exped.getId(), MaplePacketCreator.expeditionUpdate(exped.getIndex(party.getId()), party), null);
                                        }
                                        if (c.getPlayer().getEventInstance() != null) {
