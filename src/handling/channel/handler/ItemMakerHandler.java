...
public class ItemMakerHandler {
        ...
        switch (makerType) {
            case 1: { // Gem
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    c.announce(MaplePacketCreator.ItemMaker_Success());
    ...
    public static final void UseBag(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (firstTime) {
            ...
            toUse.setFlag(flag);
            c.announce(MaplePacketCreator.updateSpecialItemUse(toUse, (byte) 4, toUse.getPosition(), true, chr));
        }
        c.announce(MaplePacketCreator.openBag(chr.getExtendedSlots().indexOf(itemId), itemId, firstTime));
        c.announce(MaplePacketCreator.enableActions());
    }
...
//            c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            ...
            c.announce(MaplePacketCreator.harvestMessage(reactor.getObjectId(), GameConstants.GMS ? 13 : 11)); //ok to harvest, gogo
    ...
    public static final void ProfessionInfo(final LittleEndianAccessor slea, final MapleClient c) { //so pointless
		try {
			...
			c.announce(MaplePacketCreator.professionInfo(asdf, level1, slea.readInt(), Math.max(0, 100 - ((level1 + 1) - c.getPlayer().getProfessionLevel(Integer.parseInt(asdf))) * 20)));
    ...
    public static final void CraftEffect(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (profession != null && (c.getPlayer().getProfessionLevel(profession.intValue()) > 0 || (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
            ...
            c.announce(MaplePacketCreator.showOwnCraftingEffect(effect, time, effect.endsWith("Extract") ? 1 : 0));
