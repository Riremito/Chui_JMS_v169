...
//        mplew.writeInt(u3);
        ...
        if (slea.available() > 0) {
//            Point u4 = slea.readPos();
            Point startPos = slea.readPos();
//            int u5 = slea.readInt();
            List res;
            try {
                res = MovementParse.parseMovement(slea, 8);
    ...
    public static final void QuestAction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
            ...
            case 2: { // Complete Quest
                ...
                //c.announce(MaplePacketCreator.completeQuest(c.getPlayer(), quest));
                //c.announce(MaplePacketCreator.updateQuestInfo(c.getPlayer(), quest, npc, (byte) 14));
                // 6 = start quest
                // 7 = unknown error
                // 8 = equip is full
                // 9 = not enough mesos
    ...
    public static final void Storage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (mode) {
            case 4: { // Take Out
                ...
                if (item != null) {
                    if (!MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                        storage.store(item);
                        c.announce(MaplePacketCreator.StoragegetInventoryFull());
                    } else {
            ...
            case 5: { // Store
                ...
                if (chr.getMeso() < 100) {
                    c.announce(MaplePacketCreator.StorageMesoNotEnough(storage));
                    return;
                } else {
