...
//                player.getClient().announce(MaplePacketCreator.enableActions());
        ...
        final boolean useAttackCount = attack.skill != 4211006 && attack.skill != 3221007 && attack.skill != 23121003 && (attack.skill != 1311001 || player.getJob() != 132);
...
/*                        Item weapon_ = player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -11);
                        ...
                        if (player.getJob() == 121 || player.getJob() == 122) { // WHITEKNIGHT
                            ...
                            final Skill skill2 = SkillFactory.getSkill(1211006);
                            if (player.isBuffFrom(MapleBuffStat.WK_CHARGE, skill1)) {
                                final MapleStatEffect eff = skill1.getEffect(player.getTotalSkillLevel(skill1));
                                final MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.FREEZE, 1, skill1.getId(), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 2000, true, eff);
                            } else if (player.isBuffFrom(MapleBuffStat.WK_CHARGE, skill2)) {
                                final MapleStatEffect eff = skill2.getEffect(player.getTotalSkillLevel(skill2));
                                final MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.FREEZE, 1, skill2.getId(), null, false);
                                monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 2000, true, eff);
                            }
                        }
                    }
...
//                    if (effect != null && effect.getMonsterStati().size() > 0 && theSkill.getId() != 3101005) { // 排除炸彈箭
...
/*                    if (theSkill.getId() == 3101005 && boom == 0) {
                        if (effect.makeChanceResult()) {
                            for (Map.Entry<MonsterStatus, Integer> z : effect.getMonsterStati().entrySet()) {
                                monster.applyStatus(player, new MonsterStatusEffect(z.getKey(), z.getValue(), theSkill.getId(), null, false), effect.isPoison(), effect.getDuration(), true, effect);
                            }
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
                ...
                if (totDamageToOneMonster > 0) {
                    ...
                    // 魔力吸收
//                    if (eaterLevel > 0) {
                    if (eaterLevel > 0 && (monster.getSkills() != null || monster.getStats().getMobAttacks() != null) && !monster.getStats().isBoss()) {
...
//          }
                    ...
                    if (toCrit == 0) {
                        for (Pair<Integer, Boolean> eachd : p.attack) {
                            if (!eachd.right && hit < mid_att) {
                                if (eachd.left > 99999 || Randomizer.nextInt(100) < CriticalRate) {
                                    toCrit++;
                                }
    ...
    public static final AttackInfo parseDmgMa(final LittleEndianAccessor lea, final MapleCharacter chr) {
        ...
        lea.skip(4); // ORDER [1] byte on bigbang, [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82
    ...
    public static final AttackInfo parseDmgR(final LittleEndianAccessor lea, final MapleCharacter chr) {
        ...
        lea.skip(4); // ORDER [2] byte on bigbang [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82
        switch (ret.skill) {
        ...
        ret.csstar = (byte) lea.readShort(); // 點裝飛鏢
