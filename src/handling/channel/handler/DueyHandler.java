...
public class DueyHandler {
    ...
    public static final void DueyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (conv == 2) { // Duey
        c.announce(MaplePacketCreator.sendDuey((byte) 10, loadItems(c.getPlayer())));
        }
        break;
        }
        case 3: { // Send Item
        ...
        if (item == null) {
        c.announce(MaplePacketCreator.sendDuey((byte) 17, null)); // Unsuccessfull
        return;
        }
        ...
        c.announce(MaplePacketCreator.sendDuey((byte) 19, null)); // Successfull
        } else {
        c.announce(MaplePacketCreator.sendDuey((byte) 17, null)); // Unsuccessful
        }
        } else {
        c.announce(MaplePacketCreator.sendDuey((byte) 17, null)); // Unsuccessfull
        }
        ...
        //                              rClient.announce(MaplePacketCreator.sendDueyMSG(Actions.PACKAGE_MSG.getCode()));
        //                        }
        } else {
        c.announce(MaplePacketCreator.sendDuey((byte) 15, null)); // Same acc error
        }
        } else {
        c.announce(MaplePacketCreator.sendDuey((byte) 14, null)); // Name does not exist
        }
        } else {
        c.announce(MaplePacketCreator.sendDuey((byte) 12, null)); // Not enough mesos
        }
        break;
        }
        ...
        c.announce(MaplePacketCreator.sendDuey((byte) 16, null)); // Not enough Space
        return;
