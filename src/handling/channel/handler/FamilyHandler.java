...
public class FamilyHandler {
                ...
                if (c.getPlayer().getParty() != null ) {
                    for (MaplePartyCharacter mpc : c.getPlayer().getParty().getMembers()) {
                        if (mpc.getId() != c.getPlayer().getId()) {
                            ...
                            if (chr != null) {
                                entry.applyTo(chr);
                                //chr.getClient().announce(FamilyPacket.familyBuff(entry.type, type, entry.effect, entry.duration*60000));
                            }
                        }
                    }
                }
    ...
    public static final void FamilyOperation(final LittleEndianAccessor slea, MapleClient c) {
        ...
        if (addChr == null) {
            ...
            addChr.getClient().announce(FamilyPacket.sendFamilyInvite(c.getPlayer().getId(), c.getPlayer().getLevel(), c.getPlayer().getJob(), c.getPlayer().getName()));
        }
