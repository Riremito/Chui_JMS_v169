...
public class MapleClient implements Serializable {
    ...
    public static final byte LOGIN_NOTLOGGEDIN = 0,
            ...
            LOGIN_LOGGEDIN = 2,
            CHANGE_CHANNEL = 3;
    public static final int DEFAULT_CHARSLOT = LoginServer.getMaxCharacters()/*6*/;
    ...
    public final byte getLoginState() { // TODO hide?
        ...
        try {
            ...
            if (state == MapleClient.LOGIN_SERVER_TRANSITION || state == MapleClient.CHANGE_CHANNEL) {
                // 30秒
                if (rs.getTimestamp("lastlogin").getTime() + 30000/*20000*/ < System.currentTimeMillis()) { // connecting to chanserver timeout

                    final List<MapleCharacter> chars = loadCharacters(LoginServer.getMainServerId());
                    for (MapleCharacter chr : chars) {
//                        final boolean incs = chr.getClient().getChannel() == -10;
                        for (ChannelServer cs : ChannelServer.getAllInstances()) {
                            if (cs.getPlayerStorage().getPendingCharacter(chr.getId()) != null) {
                                chr.getClient().getSession().close();
                                chr.getClient().disconnect(true, false);
                            }
                            if (cs.getPlayerStorage().getCharacterById(chr.getId()) != null) {
                                chr.getClient().getSession().close();
                                chr.getClient().disconnect(true, false);
                            }
                        }
                        ...
                        if (CashShopServer.getPlayerStorage().getCharacterById(chr.getId()) != null) {
                            chr.getClient().getSession().close();
                            chr.getClient().disconnect(true, false);
                        }
...
//            final boolean clone = player.isClone();
            ...
            if (!fromCS) {
                ...
                try {
                    ...
                    if (RemoveInChannelServer && ch == -10/* && ch > 0*/) {
