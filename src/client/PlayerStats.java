...
/*        if (chra.getEventInstance() != null && chra.getEventInstance().getName().startsWith("PVP")) { //hack
        ...
        if (GameConstants.isDemon(chra.getJob())) {
            localmaxmp = 50; //for now; its decided by job
        }*/
...
/*        if (chr.isClone()) {
        ...
        if (changed) {
            chr.equipChanged();
            chr.getClient().announce(MaplePacketCreator.showItemLevelupEffect());
        ...
        for (Equip eqq : all) {
            if (eqq != null && eqq.getDurability() == 0 && eqq.getPosition() < 0) { //> 0 went to negative
                if (chr.getInventory(MapleInventoryType.EQUIP).isFull()) {
                    chr.getClient().announce(MaplePacketCreator.getInventoryFull());
                    chr.getClient().announce(MaplePacketCreator.getShowInventoryFull());
                    return false;
                }
...
/*    public final void handleProfessionTool(final MapleCharacter chra) {
    ...
    private final void CalcPassive_Mastery(final MapleCharacter player) {
        ...
        switch (weaponType) {
            ...
            case SWORD1H:
            case SWORD2H:
                skil = player.getJob() > 112 ? 1200000 : 1100000; //hero/pally
                break;
            case AXE1H:
            case AXE2H:
                skil = 1100001;
...
/*    private final void calculateFame(final MapleCharacter player) {
...
/*        if (chra.isClone()) {
        ...
        shouldHealMP = 3/*GameConstants.isDemon(chra.getJob()) ? 0 : (3 + mpRestore + recoverMP + (localint_ / 10))*/; // i think
        mpRecoverTime = 0;
        hpRecoverTime = 0;
	if (GameConstants.isJobFamily(100, playerjob)) { // Improving HP recovery
	    final Skill effect = SkillFactory.getSkill(1000000); // Improving HP Recovery
	    final int lvl = chra.getSkillLevel(effect);
	    if (lvl > 0) {
		shouldHealHP += effect.getEffect(lvl).getHp();
	    }
	} else if (GameConstants.isJobFamily(200, playerjob)) { // Improving MP recovery
//	    shouldHealMP += (chra.getSkillLevel(SkillFactory.getSkill(2000000)) / 10) * chra.getLevel();
            double lv = (double) chra.getSkillLevel(SkillFactory.getSkill(2000000)) / 10;
            shouldHealMP += chra.getLevel() * lv;
	}
	...
	} else if (playerjob == 121 || playerjob == 122) {
	    final Skill effect = SkillFactory.getSkill(1210000); // Improving MP Recovery
	    final int lvl = chra.getSkillLevel(effect);
	    if (lvl > 0) {
		shouldHealMP += effect.getEffect(lvl).getMp();
	    }
	} else if (playerjob == 410 || playerjob == 411 || playerjob == 412) {
	    final Skill effect = SkillFactory.getSkill(4100002); // 恢復術 - 增加自然恢復量並且在梯子和繩索上也能恢復HP/ MP。
	    final int lvl = chra.getSkillLevel(effect);
	    if (lvl > 0) {
		shouldHealHP += effect.getEffect(lvl).getHp();
		shouldHealMP += effect.getEffect(lvl).getMp();
	    }
	} else if (playerjob == 420 || playerjob == 421 || playerjob == 422) {
	    final Skill effect = SkillFactory.getSkill(4200001); // 恢復術 - 增加自然恢復量並且在梯子和繩索上也能恢復HP/ MP。
	    final int lvl = chra.getSkillLevel(effect);
	    if (lvl > 0) {
		shouldHealHP += effect.getEffect(lvl).getHp();
		shouldHealMP += effect.getEffect(lvl).getMp();
...
/*        if (playerjob == 111 || playerjob == 112) {
        ...
        if (chra.getMap() != null) { // Because Heal isn't multipled when there's a chair :)
        ...
        if ((chra.getInventory(MapleInventoryType.EQUIPPED).findById(1050018) != null || chra.getInventory(MapleInventoryType.EQUIPPED).findById(1051017) != null) && chra.getMapId() == 105040401) {
            shouldHealHP *= 1.5;
            shouldHealMP *= 1.5;
        }
        final int chair = chra.getChair();
        if (chair != 0) { // 坐椅子
            shouldHealHP *= 1.5;
            shouldHealMP *= 1.5;
            if (chair >= 3010000) {
                final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                final int RecoveryHP = ii.getRecoveryHP(chair);
                final int RecoveryMP = ii.getRecoveryMP(chair);
                if (RecoveryHP != 0) {
                    shouldHealHP = RecoveryHP;
                }
                if (RecoveryMP != 0) {
                    shouldHealMP = RecoveryMP;
                }
            }
        }
    }
