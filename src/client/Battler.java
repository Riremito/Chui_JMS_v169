...
public class Battler implements Serializable {
	...
	final long ourHp = BattleConstants.getPokemonCustomHP(monsterId, stats.getDefaultHp());
        return (long)((ourHp + Math.round(ourHp * (getLevel() - stats.getLevel()) / 50.0)) * getMod(PokemonStat.HP));
    }

    public final long calcBaseHP() {
        return (long)((stats.getDefaultHp() + Math.round(stats.getDefaultHp() * (getLevel() - stats.getLevel()) / 50.0)));
    }
...
//        getMonster().applyStatus(mse);
	...
	final int theExp = Math.max(1, stats.getDefaultExp());
