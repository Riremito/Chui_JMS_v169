...
public class GMCommand {
    ...
    public static class SP extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.announce(MaplePacketCreator.updateSp(c.getPlayer(), false));
            return 1;
        }
    }
    ...
    public static class LockItem extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (Item item : chr.getInventory(type).listById(itemid)) {
                ...
                chr.getClient().announce(MaplePacketCreator.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, chr));
            }
            if (type == MapleInventoryType.EQUIP) {
                type = MapleInventoryType.EQUIPPED;
                for (Item item : chr.getInventory(type).listById(itemid)) {
                    item.setFlag((byte) (item.getFlag() | ItemFlag.LOCK.getValue()));
                    //chr.getClient().announce(MaplePacketCreator.updateSpecialItemUse(item, type.getType()));
                }
            }
...
//            c.getPlayer().cloneLook();
            c.announce(MaplePacketCreator.spawnPlayerMapobject(c.getPlayer(), c.getPlayer().getId() + Randomizer.nextInt(c.getPlayer().getId())));
            return 1;
        }
    }
