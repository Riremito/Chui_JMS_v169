    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case item:
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        ...
                        c.getClient().announce(MaplePacketCreator.getShowItemGain(id, count, true));
                    } else { // add items
                        ...
                        if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                            final String msg1 = "勲章 - " + name + "を獲得！";
                            c.dropMessage(-1, msg1);
                            final String msg2 = "<" + name + ">勲章を獲得しました。ダリアから勲章を受け取る事ができます！";
                            c.dropMessage(5, msg2);
                        }
            ...
            case nextQuest:
                ...
                c.getClient().announce(MaplePacketCreator.updateQuestFinish(quest.getId(), status.getNpc(), intStore));
                break;
            case money:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
            ...
            case pop:
                ...
                c.getClient().announce(MaplePacketCreator.getShowFameGain(fameGain));
                break;
            case buffItemID:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        switch (type) {
            ...
            case nextQuest: {
                c.getClient().announce(MaplePacketCreator.updateQuestFinish(quest.getId(), c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
                c.gainMeso(intStore, true, true);
