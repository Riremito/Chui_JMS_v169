...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn) {
        ...
        //chr.getClient().announce(MTSCSPacket.confirmToCSInventory(ring, chr.getClient().getAccID(), csi.getSN()));
        chr.getClient().announce(MTSCSPacket.showBoughtCSItem(ring, sn, chr.getClient().getAccID()));
    }
...
//            c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
        ...
        c.announce(MaplePacketCreator.addInventorySlot(type, item));
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && (eItem.getOwner().equals(owner) || owner == null) && eItem.getExpiration() == -1) {
                                ...
                                c.announce(MaplePacketCreator.updateInventorySlot(type, eItem, false));
                            }
                        } else {
                            break;
                        }
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax) {
                                ...
                                c.announce(MaplePacketCreator.updateInventorySlot(type, nItem, false));
                            }
                        } else {
                            break;
                        }
...
//                c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                c.announce(MaplePacketCreator.moveAndMergeWithRestInventoryItem(type, src, dst, (short) ((olddstQ + oldsrcQ) - slotMax), slotMax, bag, switchSrcDst, bothBag));
            } else {
                c.announce(MaplePacketCreator.moveAndMergeInventoryItem(type, src, dst, ((Item) c.getPlayer().getInventory(type).getItem(dst)).getQuantity(), bag, switchSrcDst, bothBag));
            }
        } else {
            c.announce(MaplePacketCreator.moveInventoryItem(type, switchSrcDst ? dst : src, switchSrcDst ? src : dst, eqIndicator, bag, bothBag));
        }
    }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (stats.get("equipTradeBlock") != null || source.getItemId() / 10000 == 167) { // Block trade when equipped.
            if (!ItemFlag.UNTRADEABLE.check(flag)) {
                ...
                source.setFlag(flag);
                c.announce(MaplePacketCreator.updateSpecialItemUse_(source, MapleInventoryType.EQUIP.getType(), c.getPlayer()));
            }
        }
...
//            chr.getTrait(MapleTraitType.charm).addExp(source.getCharmEXP(), chr);
            ...
            source.setFlag(flag);
            c.announce(MaplePacketCreator.updateSpecialItemUse_(source, GameConstants.getInventoryType(source.getItemId()).getType(), c.getPlayer()));
        }
...
/*        } else if (source.getItemId() == 1122017) {
        ...
        c.announce(MaplePacketCreator.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 2, false, false));
        chr.equipChanged();
    }
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        c.announce(MaplePacketCreator.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 1, false, false));
