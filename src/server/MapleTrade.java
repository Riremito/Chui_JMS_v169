...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        exchangeMeso = 0;
        chr.get().getClient().announce(MaplePacketCreator.TradeMessage(tradingslot, (byte) 0x05/*0x07*/));
    }
    ...
    public final void cancel(final MapleClient c, final MapleCharacter chr, final int unsuccessful) {
        ...
        meso = 0;
        c.announce(MaplePacketCreator.getTradeCancel(tradingslot, unsuccessful));
    }
    ...
    public final void setMeso(final int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            chr.get().getClient().announce(MaplePacketCreator.getTradeMesoSet((byte) 0, this.meso));
            if (partner != null) {
    ...
    public final void addItem(final Item item) {
        ...
        items.add(item);
        chr.get().getClient().announce(MaplePacketCreator.getTradeItemAdd((byte) 0, item));
        if (partner != null) {
    ...
    public final void chat(final String message) {
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, CommandType.TRADE)) {
            ...
            if (partner != null) {
                partner.getChr().getClient().announce(PlayerShopPacket.shopChat(chr.get().getName() + " : " + message, 1));
            }
        }
    ...
    public final void chatAuto(final String message) {
        ...
        if (partner != null) {
            partner.getChr().getClient().announce(PlayerShopPacket.shopChat(message, 1));
        }
        if (chr.get().getClient().isMonitored()) { //Broadcast info even if it was a command.
	...
	if (c1 == null || c1.getTrade() == null) {
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c1.getClient().announce(MaplePacketCreator.serverNotice(5, "The other player is already trading with someone else."));
    ...
    public static final void visitTrade(final MapleCharacter c1, final MapleCharacter c2) {
	    ...
	    c1.getTrade().inTrade = true;
            c2.getClient().announce(PlayerShopPacket.shopVisitorAdd(c1, 1));
...
//            c2.dropMessage(-2, "System : Use @tradehelp to see the list of trading commands");
            ...
            c1.getClient().announce(MaplePacketCreator.serverNotice(5, "The other player has already closed the trade"));
        }
    }
