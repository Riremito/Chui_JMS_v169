...
public class MaplePortal {
    ...
    public final void enterPortal(final MapleClient c) {
        if (getPosition().distanceSq(c.getPlayer().getPosition()) > 60000 && !c.getPlayer().isGM()) {
...
//                c.announce(MaplePacketCreator.serverNotice(5, "前往地圖ID " + getTargetMapId() + " spawnPoint " + to.getPortal(getTarget()).getId()));
                System.out.println("前往地圖ID " + getTargetMapId() + " spawnPoint " + to.getPortal(getTarget()).getId());
