                    ...
                    public void run() {
                        ...
                        switch (turn[i]) {
                            ...
                            case ATTACK:
                                ...
                                if (battlers[i].getCurrentStatus() != null) {
                                    ...
                                    switch (s) {
                                        ...
                                        case BURN:
                                            ...
                                            battlers[i].damage((int) battlers[i].calcHP() / 16, map, battlers[i == 0 ? 1 : 0] == null ? 0 : battlers[i == 0 ? 1 : 0].getMonsterId(), true);
                                            updateHP(i);
                                            map.broadcastMessage(MaplePacketCreator.serverNotice(6, battlers[i].getName() + " was hurt " + (battlers[i].calcHP() / 16) + " damage by burn!"));
                                            break;
                                        case DARKNESS:
                                ...
                                final boolean extraEffects = false/*battlers[i].getAbility() != PokemonAbility.SheerForce && battlers[i == 1 ? 0 : 1].getAbility() != PokemonAbility.ShieldDust && s != MonsterStatus.MAGIC_CRASH*/;
                                int critChance = 20;
                                        ...
                                        if (battlers[i == 1 ? 0 : 1].getCurrentHP() <= 0) {
                                            ...
                                            if (instanceid >= 0 && i == 0 && npcTeam == null) {
                                                ...
                                                if (Randomizer.nextInt(5) == 0) {
                                                    chr.modifyCSPoints(1, (int) ((Randomizer.nextInt(chr.getClient().getChannelServer().getCashRate()) + chr.getClient().getChannelServer().getCashRate() + mons.getDefaultExp() / 1000 + mons.getDefaultHp() / 20000) * (chr.getStat().cashBuff / 100.0) * chr.getCashMod()), true);
                                                }
                                                chr.getClient().announce(MaplePacketCreator.playSound("Romio/discovery"));
                                                dispose(chr, false);
                                                return;
                                            }
                                            if (instanceid >= 0 && i == 0 && npcTeam != null) {
