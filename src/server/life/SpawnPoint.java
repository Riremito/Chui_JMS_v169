...
public class SpawnPoint extends Spawns {
    ...
    public final boolean shouldSpawn(long time) {
        ...
        // maximum of 1
        // 一個spawnpoint最多召喚(下面+1)
        if (((mobTime != 0 || !monster.getMobile()) && spawnedMonsters.get() > 0) || spawnedMonsters.get() > 2/*1*/) {
            return false;
        }
        return /*time - nextPossibleSpawn > 15000*/nextPossibleSpawn <= time;
    }

    @Override
    public final MapleMonster spawnMonster(final MapleMap map) {
        final MapleMonster mob = new MapleMonster(id, monster);
//        mob.setHp((mob.getStats().getDefaultHp() * Randomizer.rand(10, 30)) / 10);
        mob.setPosition(pos);
	mob.setCy(pos.y);
	mob.setRx0(pos.x - 50);
	mob.setRx1(pos.x + 50); //these dont matter for mobs
        if (monster.getFly()/* || monster.getMobile()*/) {
            mob.setOriginFh(fh);
        } else {
            mob.setFh(fh);
        }
	mob.setF(f);
        ...
        mob.addListener(new MonsterListener() {
            ...
            public void monsterKilled() {
                nextPossibleSpawn = System.currentTimeMillis()/* + 15000*/;
...
/*	for (MapleSummon s : map.getAllSummonsThreadsafe()) {
        ...
        if (msg != null) {
            map.broadcastMessage(MaplePacketCreator.serverNotice(1/*6*/, msg));
        }
        return mob;
    }
