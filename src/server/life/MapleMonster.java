...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private long hp, maxhp, nextKill = 0, lastDropTime = 0;
    private int mp;
...
//    private final ReentrantReadWriteLock poisonsLock = new ReentrantReadWriteLock();
    ...
    private final List<MobSkill> readydoskill = new ArrayList<>();
    ...
    private final void initWithStats(final MapleMonsterStats stats) {
        ...
        this.stats = stats;
        if (!stats.isBoss()) {
            maxhp = stats.getDefaultHp() * Randomizer.rand(70, 180) / 100;
        } else {
            maxhp = stats.getDefaultHp();
        }
        hp = maxhp/*stats.getHp()*/;
        mp = stats.getMp();
    ...
    public final int getMobExp() {
        ...
        int exp;
        if (!stats.isBoss()) {
            exp = stats.getDefaultExp() * Randomizer.rand(60, 125) / 100;
        } else {
            exp = stats.getDefaultExp();
        }
        return exp;
//        return stats.getDefaultExp();
    }
...
/*	if (poisons.size() > 0) {
    ...
    public final void switchController(final MapleCharacter newController, final boolean immediateAggro) {
        ...
        if (controllers == newController) {
            ...
            controllers.getClient().announce(MobPacket.stopControllingMonster(getObjectId()));
...
/*    public final void sendStatus(final MapleClient client) {
	if (reflectpack != null) {
        ...
        if (poisons.size() > 0) {
            poisonsLock.readLock().lock();
            try {
                client.announce(MobPacket.applyMonsterStatus(this, poisons));
            } finally {
                poisonsLock.readLock().unlock();
	    }
        }
...
/*        if (status == MonsterStatus.POISON || status == MonsterStatus.BURN) {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (stats.isBoss()) {
            if (stat == MonsterStatus.STUN || stat == MonsterStatus.POISON) {
                return;
            }
	...
	if (stats.isFriendly() || isFake()) {
        ...
        if (poison) {
            if ((getStats().getLevel() - from.getLevel()) > 10) {
                return;
            }
        }
...
/*        if (skilz != null) {
        ...
        status.setCancelTask(duration/*aniTime*/);
        if (poison && getHp() > 1) {
            status.setValue((int) getMobMaxHp() / (80 - from.getSkillLevel(status.getSkill()))); // 正服是 70 - from.getSkillLevel(status.getSkill())
            final EtcTimer tMan = EtcTimer.getInstance();
            final ScheduledFuture<?> poisonSchedule;
            poisonSchedule = tMan.register(new Runnable() {
                @Override
                    public void run() {
                    long dam = status.getX();
                    if (dam > 0) {
                        if (dam >= hp) {
                            dam = (hp - 1);
                        }
            ...
            status.setPoisonSchedule(poisonSchedule, from);
        }
	final MapleCharacter con = getController();
        stati.put(stat, status);
        if (con != null) {
            map.broadcastMessage(con, MobPacket.applyMonsterStatus(this, status, duration/*aniTime*/), getTruePosition());
            con.getClient().announce(MobPacket.applyMonsterStatus(this, status, duration/*aniTime*/));
        } else {
            map.broadcastMessage(MobPacket.applyMonsterStatus(this, status, duration/*aniTime*/), getTruePosition());
        }
/*        if (stat == MonsterStatus.POISON || stat == MonsterStatus.BURN) {
            ...
            try {
                ...
                if (con != null) {
                    ...
                    con.getClient().announce(MobPacket.applyMonsterStatus(this, poisons));
                } else {
            ...
            stati.put(stat, status);
            if (con != null) {
                map.broadcastMessage(con, MobPacket.applyMonsterStatus(this, status, duration), getTruePosition());
                con.getClient().announce(MobPacket.applyMonsterStatus(this, status, duration));
            } else {
                map.broadcastMessage(MobPacket.applyMonsterStatus(this, status, duration), getTruePosition());
	    }
        }*/
    }
...
/*    public void applyStatus(MonsterStatusEffect status) { //ONLY USED FOR POKEMONN, ONLY WAY POISON CAN FORCE ITSELF INTO STATI.
        ...
        map.broadcastMessage(MobPacket.applyMonsterStatus(this, status, (long) 100), getTruePosition());
    }*/
    ...
    public final void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration, final MobSkill skill) {
        applyMonsterBuff(effect, skillId, duration, skill, 0);
    }

    public final void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration, final MobSkill skill, final int effectDelay/*, final List<Integer> reflection*/) {
        if (effectDelay > 0) {
...
/*        for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
            ...
            final MonsterStatusEffect effectz = new MonsterStatusEffect(z.getKey(), z.getValue(), 0, skill, true);
            effectz.setCancelTask(duration + effectDelay);
            stati.put(z.getKey(), effectz);
        }*/
        ...
        for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
            if (con != null) {
                map.broadcastMessage(con, MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill, duration, effectDelay), getTruePosition());
                con.getClient().announce(MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill, duration, effectDelay));
            } else {
                map.broadcastMessage(MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill, duration, effectDelay), getTruePosition());
            }
        }
    }
...
/*        poisonsLock.readLock().lock();
    ...
    public final ArrayList<MonsterStatusEffect> getSelfPoisonBuff(final int chrid) {
        ArrayList<MonsterStatusEffect> ret = new ArrayList<MonsterStatusEffect>();
        for (MonsterStatusEffect e : stati.values()) {
            if (e.getFromID() == chrid) {
                ret.add(e);
            }
        }
        return ret;
...
/*    public final void cancelSingleStatus(final MonsterStatusEffect stat) {
	    ...
	    final MapleCharacter con = getController();
		    ...
		    map.broadcastMessage(con, MobPacket.cancelPoison(this.getObjectId(), stat), getTruePosition());
                    con.getClient().announce(MobPacket.cancelPoison(this.getObjectId(), stat));
                } else {
    ...
    public final void addreadydoskill(MobSkill skill) {
        if (!readydoskill.contains(skill)) {
            readydoskill.add(skill);
        }
    }

    public final void removereadydoskill(MobSkill skill) {
        if (readydoskill.contains(skill)) {
            readydoskill.remove(skill);
        }
    }

    public final boolean isreadydoSkill(MobSkill skill) {
        return readydoskill.contains(skill);
    }
}
