        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    ...
                    chr.getClient().announce(MaplePacketCreator.showEffect("killing/first/number/" + stage));
                    chr.getClient().announce(MaplePacketCreator.showEffect("killing/first/stage"));
                    chr.getClient().announce(MaplePacketCreator.showEffect("killing/first/start"));
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().announce(MaplePacketCreator.showEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().announce(MaplePacketCreator.showEffect("killing/fail"));
                    }
