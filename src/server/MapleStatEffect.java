                ...
                case 2300000:
                    ...
                    if (!mob.getStats().isBoss()) {
                        ...
                        if (absorbMp > 0) {
                            ...
                            applyto.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sourceid, 1, applyto.getLevel(), level));
...
//                applyto.getClient().announce(MaplePacketCreator.enableActions());
        ...
        applyto.getClient().announce(MaplePacketCreator.updatePlayerStats(hpmpupdate, true, applyto.getJob()));

        if (expinc != 0) {
            applyto.gainExp(expinc, true, true, false);
            applyto.getClient().announce(MaplePacketCreator.showSpecialEffect(11));
            ...
            if (mobid > 0) {
                ...
                if (GameConstants.GMS) {
                    applyto.getClient().announce(MaplePacketCreator.getCard(done ? sourceid : 0, 1));
                }
            }
            ...
            applyto.getClient().announce(MaplePacketCreator.getPVPScore(x + bs, false));
            ...
            applyto.getClient().announce(MaplePacketCreator.getPVPIceGage(x - iceGageCon));
            ...
            applyto.getClient().announce(MaplePacketCreator.rechargeCombo(applyto.getCombo()));
            ...
            if (portal != null) {
                applyto.getClient().announce(MaplePacketCreator.dragonBlink(portal.getId()));
            ...
            if (effectedOnEnemy > 0) {
                ...
                if (sourceid == 2910000 || sourceid == 2910001) { //red flag
                    applyto.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sourceid, 13, applyto.getLevel(), level));
                    ...
                    applyto.getClient().announce(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Effect", 0, 0));
                    ...
                    if (applyto.getTeam() == (sourceid - 2910000)) { //restore duh flag
                        ...
                        if (sourceid == 2910000) {
                            ...
                            applyto.getClient().announce(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Tail/Red", 600000, 0));
                            ...
                            applyto.getClient().announce(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Tail/Blue", 600000, 0));
...
/*            } else if (sourceid == 35111002) {
                ...
                final Map<Integer, MapleSummon> ss = applyfrom.getSummonsReadLock();
                try {
                    for (MapleSummon s : ss.values()) {
                ...
                applyfrom.getClient().announce(MaplePacketCreator.skillCooldown(sourceid, getCooldown()));
...
//                applyfrom.getClient().announce(MaplePacketCreator.enableActions()); //doubt we need this at all
            }
        } else if (isMechDoor()) {
            int newId = 0;
        ...
        if (isSoulStone()) {
            if (applyfrom.getParty() != null) {
                ...
                for (MapleCharacter chr : awarded) {
                    ...
                    chr.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected.getId() != applyfrom.getId() && (isGmBuff() || (applyfrom.inPVP() && affected.getTeam() == applyfrom.getTeam() && Integer.parseInt(applyfrom.getEventInstance().getProperty("type")) != 0) || (applyfrom.getParty() != null && affected.getParty() != null && applyfrom.getParty().getId() == affected.getParty().getId()))) {
                    if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
                        ...
                        affected.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
...
/*                    case 1111007:
    ...
    public final void applyMonsterBuff(final MapleCharacter applyfrom) {
        ...
        final List<MapleMapObject> affected = applyfrom.getMap().getMapObjectsInRect(bounds, Arrays.asList(type));
        int i = 0;
        for (final MapleMapObject mo : affected) {
            if (makeChanceResult()) {
    ...
    public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
        if (sourceid == 2311005 || sourceid == 5121005 || sourceid == 1201006 || (GameConstants.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 104)) { //doom, threaten, snatch
            ...
            chr.getClient().announce(MaplePacketCreator.giveBuff(localsourceid, getDuration(), localstatups, this));
    ...
    public final void applyComboBuff(final MapleCharacter applyto, short combo) {
        ...
        applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid, 99999, stat, this)); // Hackish timing, todo find out
        final long starttime = System.currentTimeMillis();
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        final long starttime = System.currentTimeMillis();
        if (infinity) {
            applyto.getClient().announce(MaplePacketCreator.giveEnergyChargeTest(0, duration / 1000));
            applyto.registerEffect(this, starttime, null, applyto.getId());
        } else {
            final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
    ...
    private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            ...
            case 5211007: {//dice
                ...
                applyto.getClient().announce(MaplePacketCreator.showOwnDiceEffect(sourceid, zz, -1, level));
                if (zz <= 1) {
                    return;
                }
                localstatups = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                localstatups.put(MapleBuffStat.DICE_ROLL, zz);
                applyto.getClient().announce(MaplePacketCreator.giveDice(zz, sourceid, localDuration, localstatups));
                normal = false;
                showEffect = false;
                break;
            }
            case 5320007: {//dice
                ...
                applyto.getClient().announce(MaplePacketCreator.showOwnDiceEffect(sourceid, zz, zz2 > 0 ? -1 : 0, level));
                if (zz <= 1 && zz2 <= 1) {
                    return;
                }
            ...
            case 33101006: {//jaguar oshi
                ...
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid, localDuration, localstatups, this));
                normal = false;
                break;
            }
            case 8006:
            ...
            case 15001003: {
                applyto.getClient().announce(MaplePacketCreator.givePirate(statups, localDuration / 1000, sourceid));
                if (!applyto.isHidden()) {
            ...
            case 5220011: {// Bullseye
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().announce(MaplePacketCreator.cancelHoming());
                    applyto.getClient().announce(MaplePacketCreator.giveHoming(sourceid, applyto.getFirstLinkMid(), 1));
                } else {
                    return;
                }
                normal = false;
            ...
            case 2320011: //arcane aim
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().announce(MaplePacketCreator.giveArcane(applyto.getAllLinkMid(), localDuration));
                } else {
                    return;
                }
                normal = false;
            ...
            case 32111005: { //body boost
                ...
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourcez, localDuration, stat, this));
                normal = false;
                break;
            }
            case 32001003: {//dark aura
            ...
            case 32110007: {//fallthrough intended
                ...
                statt.put(sourceid == 32110007 ? MapleBuffStat.BODY_BOOST : MapleBuffStat.AURA, (int) level);
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid, localDuration, statt, this));
                statt.clear();
            ...
            case 32120000: {
                ...
                applyto.cancelEffectFromBuffStat(MapleBuffStat.BODY_BOOST);
                final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                statt.put(MapleBuffStat.AURA, (int) applyfrom.getTotalSkillLevel(32001003));
                applyto.getClient().announce(MaplePacketCreator.giveBuff(32001003, localDuration, statt, this));
                statt.clear();
                statt.put(MapleBuffStat.DARK_AURA, x);
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), statt, this), false);
            ...
            case 32110000: {
                ...
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid == 32110000 ? 32101002 : sourceid, localDuration, statt, this));
                statt.clear();
            ...
            case 32120001: {
                ...
                applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid == 32120001 ? 32101003 : sourceid, localDuration, statt, this));
                statt.clear();
...
//            case 1211008:
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this), false);
                    applyto.getClient().announce(MaplePacketCreator.giveBuff(sourceid, localDuration, stat, this));
					...
					if (isIceKnight()) {
						...
						applyto.getClient().announce(MaplePacketCreator.giveBuff(0, localDuration, stat, this));
					}
                    final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                    stat.put(MapleBuffStat.MORPH, getMorph(applyto));
                    applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this), false);
...
//                        applyto.cancelEffectFromBuffStat(MapleBuffStat.MANA_REFLECTION);
                        applyto.getClient().announce(MaplePacketCreator.giveMount(mountid2, sourceid, stat));
        ...
        if (isMechPassive()) {
            applyto.getClient().announce(MaplePacketCreator.showOwnBuffEffect(sourceid - 1000, 1, applyto.getLevel(), level, (byte) 1));
        }
        ...
        if (normal && localstatups.size() > 0) {
            applyto.getClient().announce(MaplePacketCreator.giveBuff((skill ? sourceid : -sourceid), localDuration, maskedStatups == null ? localstatups : maskedStatups, this));
        }
        final long starttime = System.currentTimeMillis();
...
//        System.err.println("開始時間 " + starttime + " 持續時間 " + localDuration);
    }
    ...
    public static final int parseMountInfo(final MapleCharacter player, final int skillid) {
        switch (skillid) {
            ...
            case 20011004:
            case 20021004:
                if (player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -118 : -123)) != null && player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -119 : -124)) != null) {
                    return player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -118 : -123)).getItemId();
                }
    ...
    public static final int parseMountInfo_Pure(final MapleCharacter player, final int skillid) {
        switch (skillid) {
            ...
            case 20011004:
            case 20021004:
                if (player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -18 : -23)) != null && player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -19 : -24)) != null) {
                    return player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) (!GameConstants.GMS ? -18 : -23)).getItemId();
                }
                return 0;
            default:
                return GameConstants.getMountItem(skillid, player);
...
/*        if (sourceid == 4101004 || sourceid == 4201003) { // 速度激發
            return false;
        }*/
    ...
    public final boolean isHide() {
        return skill && (sourceid == 5001004/*9001004*/ || sourceid == 9101004);
    }
