...
//	rechargeableItems.add(2330006); // Beginner Bullet
    ...
    public void sendShop(MapleClient c) {
        ...
        c.announce(MaplePacketCreator.getNPCShop(getNpcId(), this, c));
    }

    public void sendShop(MapleClient c, int customNpc) {
        c.getPlayer().setShop(this);
        c.announce(MaplePacketCreator.getNPCShop(customNpc, this, c));
    }
        ...
        if (index >= 0) {
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, i.getQuantity(), i.getOwner())) {
                    ...
                    c.announce(MaplePacketCreator.confirmShopTransaction((byte) 0, this, c, x));
                } else {
                    c.getPlayer().dropMessage(1, "Your inventory is full.");
                    c.announce(MaplePacketCreator.confirmShopTransaction((byte) 0, this, c, -1));
                }
            } else {
                c.announce(MaplePacketCreator.confirmShopTransaction((byte) 0, this, c, -1));
            }
...
/*            if (GameConstants.GMS) { //TODO JUMP
            ...
            c.announce(MaplePacketCreator.confirmShopTransaction((byte) 0x4, this, c, -1));
        }
    }
    ...
    public void recharge(final MapleClient c, final byte slot) {
        ...
        if (item.getQuantity() < slotMax) {
            ...
            if (c.getPlayer().getMeso() >= price) {
                ...
                c.announce(MaplePacketCreator.updateInventorySlot(MapleInventoryType.USE, (Item) item, false));
                c.getPlayer().gainMeso(-price, false, false);
                c.announce(MaplePacketCreator.confirmShopTransaction((byte) 0x8, this, c, -1));
            }
        }
    }
