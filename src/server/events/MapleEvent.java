...
public abstract class MapleEvent {
	...
	if (GameConstants.isEventMap(chr.getMapId()) && FieldLimitType.Event.check(chr.getMap().getFieldLimit()) && FieldLimitType.Event2.check(chr.getMap().getFieldLimit())) {
	    chr.getClient().announce(MaplePacketCreator.showEventInstructions());
	}
    }
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
			    World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "Entries for the event are now closed!"));
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "Hello " + cserv.getServerName() + "! Let's play a " + StringUtil.makeEnumHumanReadable(event.name()) + " event in channel " + cserv.getChannel() + "! Change to channel " + cserv.getChannel() + " and use @event command!"));
        return "";
    }
}
